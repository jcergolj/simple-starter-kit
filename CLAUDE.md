## Guidelines Priority
When conventions from `~/work/ai-framework/agent_docs/` conflict with the Boost guidelines below, the framework conventions take precedence. Boost provides package versions, skills, and tooling info; the framework provides coding style and architectural conventions.
...
Queue::assertPushed(function (SendEmailJob $job) use ($user) {
    $this->assertTrue($job->user->is($user));

    return true;
});

Mail::assertQueued(TokenAllocated::class, function ($mail) use ($user) {
    $this->assertTrue($mail->hasTo($user->email));

    $this->assertSame('expected body', $mail->emailBody);

    $this->assertSame('expected subject', $mail->emailSubject);
